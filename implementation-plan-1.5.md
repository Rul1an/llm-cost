...
Linear BPE using backtracking with bitfield optimization.

## Algorithm

1. Pre-tokenize: split on regex pattern (whitespace, punctuation)
2. For each segment:
   a. Convert to initial byte sequence
   b. Build merge candidate list (sorted by rank)
   c. Iterate merges:
      - Find leftmost applicable merge
      - Apply merge, update adjacency
      - Mark invalidated positions in bitfield
   d. Backtrack if needed for optimal encoding
3. Concatenate segment results

## Complexity

- Time: O(N × M) where M = unique merges applied (bounded)
- Space: O(N) for bitfield + token array

## Data Structures

- `MergeTable`: hash map from (token_a, token_b) → rank
- `TokenArray`: dynamic array of current tokens
- `ValidMask`: bitfield for merge candidates

## References

- GitHub bpe crate: [link]
- BlockBPE paper: [link]
```
...
pub fn encode(
    ...
    // 1. Initialize merge table
    var merge_table = MergeTable.init(allocator);
    defer merge_table.deinit();
    try merge_table.loadFromData(merges);
    
    // 2. Pretokenize
    const segments = try pretokenizer.pretokenize(input, allocator);
    defer allocator.free(segments);
    
    // 3. Encode each segment
    var result = std.ArrayList(u32).init(allocator);
    for (segments) |segment| {
        const tokens = try encodeSegment(&merge_table, segment.bytes, allocator);
...
fn encodeSegment(
    ...
    // Apply merges until no more applicable
    while (true) {
        const best_merge = findBestMerge(merge_table, tokens.items);
        if (best_merge == null) break;
        
        applyMerge(&tokens, best_merge.?);
    }
    
    return tokens.toOwnedSlice();
}

fn findBestMerge(merge_table: *const MergeTable, tokens: []const u32) ?MergeOp {
    var best: ?MergeOp = null;
    var best_rank: u32 = std.math.maxInt(u32);
    
    var i: usize = 0;
    while (i < tokens.len - 1) : (i += 1) {
        if (merge_table.lookup(tokens[i], tokens[i + 1])) |entry| {
...
fn applyMerge(tokens: *std.ArrayList(u32), op: MergeOp) void {
    tokens.items[op.position] = op.result;
    _ = tokens.orderedRemove(op.position + 1);
}
...
pub fn countOnly(
    ...
    // Optimized path that only counts, doesn't allocate full result
    // Can use same algorithm with counter instead of array
}
...
pub fn decode(
    ...
    // Lookup each token in vocab, concatenate bytes
}
```
